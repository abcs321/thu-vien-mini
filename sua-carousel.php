...
require __DIR__ . '/queries.php';  // fetch_section_by_key(), fetch_section_images(), save_carousel_section()
...
$khoa = trim($_GET['key'] ?? 'sap_ra_mat');
if ($khoa === '') {
    $khoa = 'sap_ra_mat';
}

$section        = fetch_section_by_key($pdo, $khoa);
$existingTitle  = $section['tieu_de'] ?? 'SẮP RA MẮT';
...
<title>Chỉnh sửa mục carousel - Admin</title>
...
<div class="page-body" style="padding: 24px;">
    ...
    <h1 style="margin-bottom: 16px;">Chỉnh sửa mục carousel: <?= htmlspecialchars($existingTitle) ?></h1>
